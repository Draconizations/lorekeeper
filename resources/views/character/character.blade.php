    ...
    <div class="row mb-3">
        <div class="col-md-7">
            <div class="text-center">
                <a href="{{ $character->image->canViewFull(Auth::user() ?? null) && Storage::disk(getDisk($character->image->imageDirectory . '/' . $character->image->fullsizeFileName))->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName) ? $character->image->fullsizeUrl : $character->image->imageUrl }}"
                    data-lightbox="entry" data-title="{{ $character->fullName }}">
                    <img src="{{ $character->image->canViewFull(Auth::user() ?? null) && Storage::disk(getDisk($character->image->imageDirectory . '/' . $character->image->fullsizeFileName))->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName) ? $character->image->fullsizeUrl : $character->image->imageUrl }}"
                        class="image character-image" alt="{{ $character->fullName }}" />
                </a>
            </div>
            @if ($character->image->canViewFull(Auth::user() ?? null) && Storage::disk(getDisk($character->image->imageDirectory . '/' . $character->image->fullsizeFileName))->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName))
