            ...
            <div class="tab-pane fade {{ $image->id == $character->character_image_id ? 'show active' : '' }}" id="image-{{ $image->id }}">
                <div class="row mb-3">
                    <div class="col-md-7">
                        ...
                        @if ($image->canViewFull(Auth::user() ?? null) && Storage::disk(getDisk($image->imageDirectory . '/' . $image->fullsizeFileName))->exists($image->imageDirectory . '/' . $image->fullsizeFileName))
