    ...
    <tbody id="alleleTableBody">
        @if($alleles)
            @for($i = 0; $i < count($alleles); $i++)
                <tr id="allele-row-{{ $i }}">
                    <td>{!! Form::select('loci_id[]', $locis->pluck('name', 'id'), $loci->id, ['class' => 'form-control loci-select', 'placeholder' => 'Select Loci']) !!}</td>
                    <td class="allele-row-select">
                        {!! Form::select('allele_id[]', $loci->alleles->pluck('name', 'id'), $alleles[i]->id, ['class' => 'form-control allele-select', 'placeholder' => 'Select Allele']) !!}
                    </td>
                    <td>{!! Form::number('allele_position[]', 1, ['class' => 'form-control']) !!}</td>
