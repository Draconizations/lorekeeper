    ...
    */
    ...
    public function processImage($characterImage) {
        ...
        if (config('lorekeeper.settings.store_masterlist_fullsizes')) {
            ...
            $disk->put(
                $characterImage->imageDirectory.'/'.$characterImage->fullsizeFileName,
                $image->encode(config('lorekeeper.settings.masterlist_fullsizes_format') ?? $characterImage->fullsize_extension, 100)
            );
        } else {
    ...
    private function handleCharacterImage($data, $character, $isMyo = false) {
        try {
            if ($isMyo) {
                ...
                if (!isset($data['image'])) {
                    $data['image'] = 'images/myo.png';
                    $data['thumbnail'] = 'images/myo-th.png';
